...
namespace MyFirstWeb.Tests
{
    public abstract class MyFirstWebTestBase : AbpIntegratedTestBase<MyFirstWebTestModule>
    {
        ...
        protected void UsingDbContext(Action<MyFirstWebDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyFirstWebDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyFirstWebDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyFirstWebDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyFirstWebDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyFirstWebDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyFirstWebDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyFirstWebDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyFirstWebDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyFirstWebDbContext, Task<T>> func)
        {
            T result;
